...
class MatchEngine(IMatchVacancyUseCase):
    ...
    def match_vacancy(self, vacancy_id: str) -> Candidate | None:
        ...
        with self._uow:
            # Retrieve vacancy
            vacancy = self._uow.repository.get_vacancy(vacancy_id)
            if not vacancy:
            ...
            queue = self._uow.repository.get_queue(vacancy.profession_code)
            if not queue:
            ...
            for c_id in queue.candidate_ids:
                candidate = self._uow.repository.get_candidate(c_id)
                if not candidate:
                    continue
